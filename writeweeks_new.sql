		...
		for (var firstWeek = 1; firstWeek <= firstWeekOfYear.length; firstWeek++) {
			...
			for (var day = 1; day <= firstDayOfWeek.length; day++) {
				...
				var insertString = "INSERT INTO casdev01.period (datefrom, datethru, periodtype, ";
					...
					insertString += "," + dayNameString + "," + firstWeek + "," + weekNumber + ", 1, " + year + ")";
...
function getDayName (day, dayName) {
	...
	if (day == 1) {
		dayNameString = dayName[0];
	} else if (day == 2) {
		dayNameString = dayName[1];
	} else if (day == 3) {
		dayNameString = dayName[2];
	} else if (day == 4) {
		dayNameString = dayName[3];
	} else if (day == 5) {
		dayNameString = dayName[4];
	} else if (day == 6) {
		dayNameString = dayName[5];
	} else if (day == 7) {
		dayNameString = dayName[6];
	}
